...
from test_rl_macd import StockRL
import os.path
...
class StockScore:
    ...
    def checkRec(self):
        ...
        #6. G20 / G1 > 2
            ...
            reason = "G20/G1 > 2"
        #7. Healthcare & G20Year < 5
        elif (self.company['industry'] == "Pharmaceuticals" or self.company['industry'] == "Healthcare Equipment & Supplies" or self.company['industry'] == "Healthcare Providers & Services"\
    ...
    def checkRL(self):
        ...
        reason = ''
        #1. model_score < 1.1
        if self.result['rl_result']['model_score'] < 1.05:
            #rec = "n"
            ...
            #3. len < 5
            elif self.result['rl_result']['predict_macd_len'] < 5:
                #rec = "n"
                reason = "len < 5"
            #4. predict_vol = 0
    ...
    def targetGrowthRate(self, lookforward):
        ...
        if len(q) > 0:
            ...
            #print(startDate, endDate)
            tYear = (datetime(*(int(s) for s in startDate.split('-'))) - datetime(*(int(s) for s in endDate.split('-')))).days / 365
            weight = 100
            totalT = start / end - 1
            if tYear != 0:
    ...
    def run(self):
        ...
        if self.result['Recommendation'] == '' and self.q is not None and len(self.q) > 0 and self.q[0] is not None:
            ...
            #compute final score
            s = cap['capScore'] + peScore['PEScore'] + g20['G'] + rank
        ...
        if self.result['Recommendation'] == '':
            ...
            ss = StockStats(self.symbol)
            self.result['MACD'] = ss.macd_by_date(self.AAOD, short, long, signal)
            #print(ss.stock)

            # get StockRL
            rl = StockRL(self.symbol, 0, short, long, signal)
            file_path = './rl/test_rl_' + self.symbol + '.zip'
            rl.retrain() if os.path.exists(file_path) else rl.train()
